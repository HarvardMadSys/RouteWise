...
Used by the simulation runners' optional ``--predictor`` flag to support
``constant_<kind>`` (e.g., ``constant_mean``, ``constant_p99``) and
``fixed:<value>`` baselines alongside the streaming predictors.
"""
...
def workload_constant_value(requests: Iterable[Request], *, kind: str) -> float:
    ...
    ``kind`` is one of ``mean``, ``p1``, ``p10``, ``p25``, ``p50``, ``p75``,
    ``p90``, ``p99``, or ``fixed:<value>`` for a literal token count.
    """
    ...
    raise ValueError(
        f"unknown constant predictor kind {kind!r}; "
        "expected one of mean, p1, p10, p25, p50, p75, p90, p99, fixed:<value>"
    )
