"""Transport abstraction for Phase 6 joint online evaluation.

Each transport knows how to send a single streaming chat-completion request
to one of the supported provider backends and parse TTFT + token usage.

Supported backends:
    - openrouter       : OpenRouter (optionally with provider hint)
...
All transports return a common SingleRequestResult dataclass so the main
harness can stay transport-agnostic.
"""
...
# ---------------------------------------------------------------------------
# Default endpoints. These can be overridden via environment variables.
# ---------------------------------------------------------------------------
...
class SingleRequestResult:
    """Result of a single request, transport-agnostic."""
...
class OpenAICompatStreamingTransport(BaseTransport):
    """Covers OpenAI-compatible chat-completions streaming endpoints.

    Used by chutes, featherless, minimax_native, ollama_cloud, and openrouter.
    The provider-specific differences are handled via cfg.model,
    cfg.provider_hint (openrouter only), and cfg.extra_headers.
    """
    ...
    def send(
        ...
        try:
            ...
            for raw_line in response.iter_lines(decode_unicode=True):
                ...
                # OpenRouter reports the picked subprovider in chunk-level "provider".
                ...
                if choices:
                    ...
                    # Reasoning models (e.g. MiniMax-M2.5) stream visible tokens
                    # in delta.reasoning / reasoning_content / thinking before
                    # the final answer appears in delta.content. Treat any
                    # non-empty stream field as the user-visible first token.
                    visible_token = (
                        delta.get("content")
        ...
        # Prefer the OpenRouter-reported actual cost when present (handles the
        # case where openrouter_auto / sort=latency pick a sub-provider whose
...
def build_transport(cfg: TransportConfig, session: requests.Session) -> BaseTransport:
    ...
    if cfg.transport in ("openrouter", "chutes", "featherless", "minimax_native", "ollama_cloud"):
...
def resolve_transport_config(provider_entry: dict[str, Any]) -> TransportConfig:
    ...
    if transport == "openrouter":
        ...
        extra_headers = {
            ...
            "X-Title": "RouteWise phase6 joint evaluation",
        }
        ...
        base_url = _ensure_v1_suffix(os.environ.get("FEATHERLESS_BASE_URL", "https://api.featherless.ai"))
