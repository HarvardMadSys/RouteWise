...
# Import from the existing router module.
# Use importlib to avoid triggering experiment.strategies.__init__.py
# which pulls in unrelated modules with missing dependencies.
import importlib.util as _ilu
import os as _os
import sys as _sys

def _load_router_module():
    _path = _os.path.join(_os.path.dirname(__file__), "online_latency_router.py")
    _spec = _ilu.spec_from_file_location("_olr_for_v2", _path)
    _mod = _ilu.module_from_spec(_spec)
    _sys.modules[_spec.name] = _mod
    _spec.loader.exec_module(_mod)
    return _mod

_olr = _load_router_module()
FailureMode = _olr.FailureMode
ProviderProfile = _olr.ProviderProfile
pre_filter = _olr.pre_filter
