...
from .scenarios import TieredScenarioConfig  # noqa: E402
from .shadow_price import (  # noqa: E402
    calibrate_envelopes,
...
# V3 (Bernoulli, profile-based): miss_rate UCB must be <= alpha to admit.
# alpha is the maximum tolerable SLO miss probability; 0.05 means we
# require F_j(SLO) >= 0.95 with probability 1 - delta.
_ALPHA_SLO_MISS = 0.05

# Confidence level for the Clopper-Pearson upper bound. For anytime-valid
# bounds one should use time-varying delta (sum 1/t^2 trick) or a Howard-
# Ramdas confidence sequence. Here we use a fixed delta in the simulator
# to avoid penalizing long runs; we note this limitation in the design
# doc (production would need anytime-valid bounds).
_DELTA_BASE = 0.05
_USE_TIME_VARYING_DELTA = False

# Exploration bonus strength for the joint_ucb variant (kappa in the design note).
_KAPPA_EXPLORATION = 0.3

# Probe rate for S_A providers only. S_Q / S_C are NOT actively probed
# (see JOINT_DESIGN_v2 section 3.1). Set to 0.0 to disable active probing
# entirely and rely only on feedback + hedge-as-probe.
_PROBE_RATE_S_A = 0.05

# Warm-up: pre-populate each provider's profile with this many samples
# spread over the 14 minutes preceding t=0. Needs to be large enough that
# a CP UCB at zero observed misses can certify miss_rate <= alpha.
# For alpha=0.05 / delta=0.05, n >= 59 is the minimum from CP inversion.
# We use 200 to give comfortable margin and match production pre-launch
...
# ---------------------------------------------------------------------------
# joint_ucb: Bernoulli profile + Clopper-Pearson safety filter
# ---------------------------------------------------------------------------
...
def _warm_up_profiles(
    ...
    """Pre-populate each provider's profile with persistent prior counts.

    Warm-up samples are drawn from the provider's true distribution; we
    count hits / misses against the SLO and install them as a persistent
    `prior` on the profile. They do not occupy the rolling window (they
    never expire), so rarely-routed providers retain a stable estimate
    over long runs.

    In production, this prior corresponds to pre-deployment benchmarking
    that establishes each provider's baseline SLO miss probability.
    """
    for p in providers:
        prof = profiles[p.name]
        hits = 0
        misses = 0
        for _ in range(_WARM_UP_N_PER_PROVIDER):
            ttft = p.sample_ttft(rng, t0)
            if ttft <= slo_ms:
                hits += 1
            else:
                misses += 1
        prof.add_prior(hits, misses)


def _joint_select_ucb(
    providers: list[TieredProvider],
    profiles: dict[str, ProviderProfile],
    req: Request,
    now: float,
    ...
    U: float,
    L: float,
    alpha: float = _ALPHA_SLO_MISS,
    delta_base: float = _DELTA_BASE,
    kappa: float = _KAPPA_EXPLORATION,
    t_index: int = 1,
    latency_alpha: float = 0.0,
) -> tuple[TieredProvider, dict[str, float]]:
    """UCB-style cross-tier selection using Bernoulli miss-rate profile.

    Safety filter (pessimistic UCB):
        admit j  iff  miss_rate_ucb_j(delta_t) <= alpha

    Scoring:
        score_j = c_eff[j] - kappa * U * scarcity_weight * uncertainty_radius

    where:
        scarcity_weight = max(0, 1 - (psi(z) + lambda(u)) / U)
        uncertainty_radius = miss_rate_ucb - miss_rate_hat

    The bonus shrinks (a) as n_j grows (radius shrinks) and (b) as the
    provider's shadow price approaches U (scarcity weight shrinks).
    """
    candidates = [p for p in providers if p.is_available(now)]
    if not candidates:
        s_a = [p for p in providers if p.tier == ProviderTier.S_A]
    ...
    if _USE_TIME_VARYING_DELTA:
        n_k = max(len(providers), 1)
        delta_t = delta_allocation(delta_base, n_k, float(t_index))
    else:
        delta_t = delta_base

    # Safety filter: miss_rate UCB <= alpha.
    safe = []
    for p in candidates:
        if profiles[p.name].miss_rate_ucb(delta_t) <= alpha:
            safe.append(p)

    if not safe:
        # Cold start / all excluded. Fall back to the candidate whose UCB
        # is lowest (best shot at meeting SLO). An explicit canary-budget
        # policy would go here in production; for the simulator we admit
        # the single best-looking candidate.
        safe = [min(candidates, key=lambda p: profiles[p.name].miss_rate_ucb(delta_t))]

    # Effective cost + exploration bonus.
    c_eff_map = {
        p.name: effective_cost(
            p, req.total_tokens, now, U=U, L=L, latency_alpha=latency_alpha,
        )
        for p in safe
    }
    ...
    for p in safe:
        c = c_eff_map[p.name]
        shadow = (
            quota_shadow_price(p, now, U=U, L=L)
            + concurrency_shadow_price(p, now, U=U)
        )
        scarcity_weight = max(0.0, 1.0 - shadow / U)
        rho = profiles[p.name].uncertainty_radius(delta_t)
        bonus = kappa * U * scarcity_weight * rho
        scores[p.name] = c - bonus

    primary = min(safe, key=lambda p: scores[p.name])
    return primary, c_eff_map
...
def _run_joint_ucb(
    ...
    *,
    ...
    """Joint routing with Bernoulli-profile safety filter and UCB exploration.

    Profile updates come from three sources:
        1. Primary feedback (always).
        2. Hedge-as-probe: if a hedge fires, backup response feeds profile.
        3. S_A-only active probing at 5% rate (paid probes for S_A tier).
    """
    _reset_all_state(scenario.providers)
    ...
    # Initialize Bernoulli profiles and warm them up with pre-run samples.
    profiles: dict[str, ProviderProfile] = {
        p.name: ProviderProfile(slo_ms=slo_ms, window_sec=900.0)
        for p in scenario.providers
    }
    t0 = float(requests[0].timestamp) if requests else 0.0
    _warm_up_profiles(profiles, scenario.providers, t0, slo_ms, rng)
    ...
    for t_index, req in enumerate(requests, start=1):
        t = float(req.timestamp)

        primary, c_eff_map = _joint_select_ucb(
            scenario.providers, profiles, req, t, slo_ms,
            U=U, L=L, t_index=t_index,
        ...
        hedged = False
        final_ttft_ms = T_primary_ms
        cost = primary.marginal_cost(req.total_tokens, t)

        if use_hedge:
            backup = _pick_cross_tier_backup(scenario.providers, primary, t)
            if backup is not None:
                primary_p50 = profiles[primary.name].p50_estimate() or 200.0
                wait_threshold_ms = max(primary_p50 * 1.5, slo_ms * 0.5)
                if T_primary_ms > wait_threshold_ms:
                    P_viol = 0.5
                    remaining_ms = slo_ms - wait_threshold_ms
                    backup_p50 = profiles[backup.name].p50_estimate() or 200.0
                    F_backup = 1.0 if backup_p50 < remaining_ms else 0.3
                    c_backup_eff = c_eff_map.get(
                        backup.name,
                        effective_cost(
                            backup, req.total_tokens, t, U=U, L=L,
                            latency_alpha=latency_alpha,
                        ),
                    )
                    if P_viol * F_backup > c_backup_eff / V_penalty:
                        ...
                        cost += backup.marginal_cost(req.total_tokens, t)
                        hedged = True
                        # Hedge-as-probe: backup response is a free observation.
                        profiles[backup.name].add_sample(t, T_backup_ms)

        # Feedback into primary's profile.
        profiles[primary.name].add_sample(t, T_primary_ms)

        # Optional S_A probing (NOT for S_Q or S_C).
        for p in scenario.providers:
            if p.name == primary.name:
                continue
            if (
                p.tier == ProviderTier.S_A
                and p.is_available(t)
                and rng.random() < _PROBE_RATE_S_A
            ):
                probe_ttft = _sample_ttft(p, rng, t)
                profiles[p.name].add_sample(t, probe_ttft)
