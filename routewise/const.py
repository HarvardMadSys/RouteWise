...
`routewise/sim/engine/simulator.py` for the simulator's physical model) was exactly
...
#
#   1. `routewise/core/hedging.py:combined_success_probability` subtracts it
#      from `remaining_ms` when estimating how much time a simulated backup has
#      after a hedge decision.
#   2. `routewise/sim/engine/simulator.py:_execute_request` adds it to simulated
...
#
#   1. `routewise/core/hedging.py` — shared hedging math.
...
#
#   1. `routewise/core/hedging.py:hedge_checkpoints_for_slo` — canonical
