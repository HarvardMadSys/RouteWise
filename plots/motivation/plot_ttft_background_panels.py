...
def plot_ttft_share_panel(ax: plt.Axes, output_input_ratio: float) -> dict[str, Any]:
    ...
    ax.legend(loc="lower right", frameon=False, handlelength=1.2, borderaxespad=0.1)
    ax.text(-0.23, 1.05, "(a)", transform=ax.transAxes, fontsize=9.0, fontweight="bold")
    return summary
...
def plot_context_length_panel(ax: plt.Axes, source_csv: Path) -> list[dict[str, float | int | str]]:
    ...
    compact_labels = [str(item["prompt_bucket"]).replace("K", "") for item in summary]
    ...
    ax.set_xlabel("Prompt tokens (K)", labelpad=0.8)
    ...
    ax.text(-0.21, 1.05, "(b)", transform=ax.transAxes, fontsize=9.0, fontweight="bold")
    return summary
...
def plot_combined(
    ...
    fig.subplots_adjust(left=0.13, right=0.995, bottom=0.28, top=0.93, wspace=0.52)
