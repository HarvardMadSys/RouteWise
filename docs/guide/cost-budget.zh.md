...
RouteWise 负责通用的实际复用证据语义：正面、负面和未知观测，TTL 与指数衰减，学习到的
缓存 token 估计，以及对路由/成本和调用方估计优先级的影响。它只了解 `provider.name` 和
不透明的 `affinity_key`。应用可以维护更细粒度的会话、前缀、端点或凭证局部性模型，而不应
把 RouteWise 的粗粒度证据当作权威缓存状态。
