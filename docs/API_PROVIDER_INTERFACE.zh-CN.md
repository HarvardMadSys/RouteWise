...
> 契约评审。范围仅限 API 提供商；initial release（package `0.1.0`）是论文
...
> 原型实现，包括表 A/表 B。公共 API 冻结与 `0.1.0` 发布，以其余开放问题
> 全部关闭且 release gates 通过为准。它们所依赖的数学原语目前已经存在，
> 并记录在 [CORE_API.md](CORE_API.md) 中。本文档将该接口
> 命名为“API v1”；首次交付它的 package 版本是 `0.1.0`，关于 `1.0.0` 的
...
14. `Client` 和 LiteLLM plugin 移至后续 release；initial release
    （`0.1.0`）只交付无依赖的 `Router`。等待 Juncheng 签字确认。
...
17. initial implementation 会预留一个内部 capacity transaction 接缝，
    `0.1.0` 中仅由 no-op API controller 实现。Capacity admission 与 L/U
...
5. billing 是每 attempt 一台三状态机，状态集合为 `{unknown, calculated,
   ...
   前者相加。
6. `observe()` 在 `0.1.0` 中去掉 `at=`：所有 observation 都由 router 的
...
13. router 不持有任何未终结 handle 的强引用；attempt 状态存放在 handle
    ...
    reservation（`0.1.0` 中为 no-op）为上界。
14. `0.1.0` 定名为 API-only preview。论文的主要结果包含 quota 与
...
计划中的 initial release（package `0.1.0`）是 API-only preview：只包含决策库
...
作为可选 extra 提供；`0.1.0` 不定义任何 extra。参见“范围与路线图”。
...
6. router 不持有任何未终结 handle 的强引用：attempt 状态存放在 handle 上，
   ...
   reservation（`0.1.0` 中为 no-op）；被丢弃的 handle 会被垃圾回收，router
   侧残留不超过上述上界。
...
每条 observation 都由 router 的时钟盖“现在”戳；`0.1.0` 中没有 `at=`，因此暂不
...
| `observe()` 时间戳 | 当前公开操作的唯一 `now` | 一律“现在”；`0.1.0` 无 `at=` |
...
也是计划中的 v2 扩展；论文的主要结果依赖它们，这正是 `0.1.0` 定名为
...
initial `0.1.0` public surface 仍然仅支持 API，不暴露
...
1. `Client` 是随 initial release（`0.1.0`）发布，还是推迟到后续版本？当前
   草案选择推迟。如果移入 `0.1.0`，契约还必须把 `base_url` 和
...
10. wheel 是否保留研究子包（`llm_routewise.sim`、`llm_routewise.offline`、
    ...
    但能让研究用户免于 source checkout。保留还会击穿“`0.1.0` 零 extras”
...
7. 私有 capacity seam、no-op controller、reservation 状态机和有界
   ...
   `llm_routewise.core`，这些 capacity Protocol 也不属于 `0.1.0` compatibility
   surface。
...
`[project.scripts]` 的 CLI entry point 不进入 `0.1.0`：当前
`routewise_cli.main` 在模块顶层导入 `experiments`，因此在出现 library-only
CLI 之前，console command 从 wheel 中移除。`0.1.0` 不定义任何 pip extra；
`[client]` 与 `[litellm]` 随其功能一同到来。

| 发布门 | 要求 | 当前状态 |
| --- | --- | --- |
| wheel 内容 | 按上述 allowlist；不含 experiments、CLI 或数据 | 本地通过：25 个条目，精确 member 检查通过 |
| console script | `0.1.0` 不携带 | 通过：entry point 已移除，source CLI 仅供仓库使用 |
...
| 元数据 | `version = 0.1.0`；库定位 description；README library-first；`[project.urls]`、classifiers、SPDX license；arXiv 引用 | 除 arXiv 引用外均已完成 |
