...
- `None` (omitted): no caller estimate; learned affinity evidence may fill
  estimates when `affinity_key` is supplied. Without an affinity key, no
  learned estimate is available and the cost model uses its no-cache baseline.
...
- **`None` means unknown**: `cached_tokens=None` means no authoritative
  cache-use observation was available. It is not `cached_tokens=0`; no learned
  positive or negative evidence is created from a missing observation, and
  successful dispatch alone does not create evidence.
