...
> (package `0.1.0`) is an **API-only preview** of the paper's system. Revision
...
> The public API freezes, and `0.1.0` ships, only after the remaining open
...
> "API v1"; the package that first ships it is `0.1.0`, and any `1.0.0`
...
14. `Client` and the LiteLLM plugin move to a later release; the initial
    release (`0.1.0`) ships the dependency-free `Router` only. Pending
    Juncheng's sign-off.
...
17. The initial implementation reserves an internal capacity-transaction
    seam, backed only by a no-op API controller in `0.1.0`. Capacity admission
...
6. `observe()` loses `at=` in `0.1.0`: every observation is stamped "now" by
...
13. The router holds no strong references to outstanding handles; attempt
    ...
    `0.1.0`).
14. `0.1.0` is named an API-only preview. The paper's headline results
...
The planned initial release (package `0.1.0`) is an API-only preview: it
...
optional extras for a later release; `0.1.0` defines no extras. See Scope and
Roadmap.
...
6. The router holds no strong reference to any outstanding handle: attempt
   ...
   `0.1.0`), and a dropped handle is garbage-collected with no router-side
...
`0.1.0`, so replaying history with original timestamps is not supported yet.
...
| `observe()` stamp | the current operation's single `now` | always "now"; no `at=` in `0.1.0` |
...
extension; the paper's headline results depend on them, which is why `0.1.0`
...
The initial `0.1.0` public surface remains API-only and does not expose
...
1. Does `Client` ship in the initial release (`0.1.0`) or a later one? The
   draft defers it. If it moves into `0.1.0`, the contract must also add
...
10. Does the wheel keep the research subpackages (`llm_routewise.sim`,
    ...
    "`0.1.0` defines no extras" stance: the research subpackages pull
...
7. The private capacity seam, no-op controller, reservation state machine, and
   ...
   `llm_routewise.core`, and the capacity Protocols are not part of the `0.1.0`
...
`llm_routewise.metrics`. The `[project.scripts]` CLI entry point does not ship in `0.1.0`:
...
`0.1.0` defines no pip extras; `[client]` and `[litellm]` arrive with their
features.
...
| Console script | absent from `0.1.0` | passes: entry point removed; source CLI is repository-only |
...
| Metadata | `version = 0.1.0`; library description; README library-first; `[project.urls]`, classifiers, SPDX license; arXiv citation resolved | partial: everything except the arXiv citation is present |
