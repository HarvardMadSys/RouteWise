"""QuotaState rolling-window regressions shared by simulator and real eval."""

from __future__ import annotations

from routewise.capacity import QuotaState
...
def test_first_charge_snaps_to_fixed_boundary() -> None:
    ...
    """
    q = QuotaState(window_sec=3600.0, size=10)
...
def test_advances_by_full_window_multiples_under_sparse_traffic() -> None:
    ...
    q.roll_window(now=1000.0 + 2.5 * 3600.0)
    assert q.used == 0
...
def test_fraction_used_within_window() -> None:
    q = QuotaState(window_sec=60.0, size=4)
    q.charge(now=120.0)
    q.charge(now=121.0)
...
def test_can_admit_blocks_at_limit() -> None:
    q = QuotaState(window_sec=60.0, size=2)
    q.charge(now=10.0)
    q.charge(now=11.0)
    assert q.can_admit(now=12.0) is False
    # New window: counter resets, slot frees.
    assert q.can_admit(now=80.0) is True
