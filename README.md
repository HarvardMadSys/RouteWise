...
- **`None` means unknown**: `cached_tokens=None` is not an explicit cache miss;
  no evidence is created from a missing observation, and dispatch alone does
  not establish cache warmth.
