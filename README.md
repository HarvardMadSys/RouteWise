...
data: `data/real_eval_records/` holds the request-level records of all ten
policy runs (see its README for the schema), and one command rebuilds every
real-world figure and the summary table from them:

```bash
uv run python -m plots.end_to_end.plot_real_world_frontier \
    --input-dir data/real_eval_records \
    --billing-duration-sec 86400 --fixed-cost-non-or 1.5476333333333334 \
    --routewise-plot-alphas 0 0.25 0.5 0.75 1 \
    --mean-ttft-out outputs/figures/fig1a_mean_ttft.pdf \
    --slo-out outputs/figures/fig1b_slo_violations.pdf \
    --table-out outputs/figures/real_world_rows.tex \
    --summary-out outputs/figures/real_world_summary.json \
    --boxplot-out outputs/figures/fig6a_ttft_distribution.pdf \
    --boxplot-policies budget_range_alpha0_hedge budget_range_alpha25_hedge \
        budget_range_alpha50_hedge budget_range_alpha75_hedge budget_range_alpha100_hedge \
        greedy_cost greedy_latency or_auto or_sort_latency or_sort_cost \
    --provider-mix-out outputs/figures/fig6b_provider_mix.pdf \
    --provider-mix-policies budget_range_alpha0_hedge budget_range_alpha25_hedge \
        budget_range_alpha50_hedge budget_range_alpha75_hedge budget_range_alpha100_hedge \
        greedy_cost greedy_latency or_auto or_sort_latency or_sort_cost \
    --provider-latency-out outputs/figures/fig7a_provider_latency.pdf \
    --provider-latency-values-json plots/end_to_end/paper_minimax_provider_latency.json \
    --provider-pricing-out outputs/figures/fig7b_provider_pricing.pdf
```

The two billing parameters prorate the subscription fees over the 24-hour
replay window as the paper does. `real_world_summary.json` should match
`data/real_eval_records/reference_summary.json` exactly (per policy: total
cost, mean and P99 TTFT, SLO-violation rate); for example RouteWise at
α = 0 costs $2.183 with a mean TTFT of 0.924 s and 0.45% SLO violations,
and OR-price costs $2.060 with 2.360 s and 20.66%. Figure 7a plots the
provider mean-TTFT values recorded with the paper run
(`paper_minimax_provider_latency.json`); Figure 7b plots the provider prices
from the run's inventory file.
...
(`data/freeinference.jsonl`; arrival times, token counts, and per-account
pseudonyms only — see `data/README.md`) is committed. Replay it through the
end-to-end module on the paper's `end_to_end_rw8` scenario and rebuild the
four Figure 8 panels (~1 minute for the replay):

```bash
uv run python -m experiments.simulation.end_to_end \
    --scenario end_to_end_rw8 --workload freeinference --prefix-cache-enabled --jobs 8
